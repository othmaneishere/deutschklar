...
import { EXPANDED_VOCAB_THEMES, VocabTheme, VocabCard } from '../data/vocabThemesData';
...
export const VocabularyLibrary: React.FC<VocabularyLibraryProps> = ({
  ...
  const [selectedThemeId, setSelectedThemeId] = useState<string>(EXPANDED_VOCAB_THEMES[0].id);
  ...
  const filteredThemes = useMemo(() => {
    return EXPANDED_VOCAB_THEMES.filter((t) => {
  ...
  const allCards = useMemo(() => {
    return EXPANDED_VOCAB_THEMES.flatMap((t) => t.cards);
  }, []);
  ...
  const totalWordsCount = useMemo(() => {
    return EXPANDED_VOCAB_THEMES.reduce((acc, t) => acc + t.cards.length, 0);
  }, []);
        ...
        <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 shrink-0">
            <div className="bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl p-3.5 text-center">
              <div className="text-2xl font-black text-indigo-400">{EXPANDED_VOCAB_THEMES.length}</div>
        ...
        <div className="bg-white rounded-3xl border border-slate-200/90 p-6 sm:p-10 shadow-sm space-y-8">
          ...
          <div className="flex items-center justify-between gap-4 flex-wrap bg-slate-50 p-3 rounded-2xl border border-slate-200">
            ...
            <div className="flex items-center gap-2">
              ...
              >
                Alle {EXPANDED_VOCAB_THEMES.length} Themen ({allCards.filter((c) => !!c.article).length} Wörter)
              </button>
            </div>
          </div>
